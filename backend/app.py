...
def conectar():
    return pymysql.connect(host='localhost', user='root', password='root', database='greeco', charset='utf8mb4')

# ----------------- CRUD para la tabla tipo_energia -----------------


# Ruta para consulta general de la tabla Fuentes
@app.route("/")
def consulta_general():
    try:
        conn = conectar('localhost', 'root', 'root', 'energia_limpia')
        cur = conn.cursor()
        ...
        for row in datos:
            dato = {
                'id_tipo_energia': row[0],
                'tipo': row[1],
                'nombre_tipo_energia': row [2],
            }
            data.append(dato)
        cur.close()
        conn.close()
        return jsonify({'tipo_energia': data, 'mensaje': 'Consulta exitosa'})
...
def obtener_investigadores():
    try:
        ...
        cursor = conexion.cursor(pymysql.cursors.DictCursor)
        query = """
            SELECT i.id_investigador, i.nombre, i.apellido, i.especialidad, 
                   p.nombre AS proyecto
            FROM Investigadores i
            LEFT JOIN Proyecto p ON i.id_proyecto = p.id_proyecto
        """
        cursor.execute(query)
        investigadores = cursor.fetchall()
        return jsonify({'investigadores': investigadores}), 200
    except Exception as e:
        return jsonify({'mensaje': 'Error al obtener investigadores', 'error': str(e)}), 500
    finally:
        cursor.close()
        conexion.close()
...
def agregar_investigador():
    try:
        ...
        conexion = conectar()
        cursor = conexion.cursor()
        query = "INSERT INTO Investigadores (nombre, apellido, especialidad, id_proyecto) VALUES (%s, %s, %s, %s)"
        cursor.execute(query, (nombre, apellido, especialidad, id_proyecto))
        conexion.commit()

        return jsonify({'mensaje': 'Investigador agregado exitosamente'}), 201
    except Exception as e:
        return jsonify({'mensaje': 'Error al agregar investigador', 'error': str(e)}), 500
    finally:
        cursor.close()
        conexion.close()
@app.route('/investigadores/<int:id_investigador>', methods=['PACHT'])
def editar_investigadores(id_investigador):
    try:
        datos = request.json
        nombre = datos.get('nombre')
        apellido = datos.get('apellido')
        ...
        query = """
            UPDATE Investigadores 
            SET nombre = %s, apellido = %s, especialidad = %s, id_proyecto = %s 
            WHERE id_investigador = %s
        """
        cursor.execute(query, (nombre, apellido, especialidad, id_proyecto, id_investigador))
        conexion.commit()
        ...
        cursor.close()
        conexion.close()
@app.route('/investigadores/<int:id_investigador>', methods=['DELETE'])
def eliminar_investigador(id_investigador):
    try:
        conexion = conectar()
        cursor = conexion.cursor()
        query = "DELETE FROM Investigadores WHERE id_investigador = %s"
        cursor.execute(query, (id_investigador,))
        conexion.commit()
        ...
        return jsonify({'mensaje': 'Error al eliminar investigador', 'error': str(e)}), 500
    finally:
        cursor.close()
        conexion.close()
@app.route('/investigadores/<int:id_investigador>', methods=['GET'])
def obtener_investigador_por_id(id_investigador):
    try:
        conexion = conectar()
        cursor = conexion.cursor(pymysql.cursors.DictCursor)
        query = """
            ...
            FROM Investigadores i
            LEFT JOIN Proyecto p ON i.id_proyecto = p.id_proyecto
            WHERE i.id_investigador = %s
        """
        cursor.execute(query, (id_investigador,))
        investigador = cursor.fetchone()

        if investigador:
            return jsonify({'investigador': investigador}), 200
        else:
            return jsonify({'mensaje': 'Investigador no encontrado'}), 404
    except Exception as e:
        return jsonify({'mensaje': 'Error al obtener investigador', 'error': str(e)}), 500
    finally:
        cursor.close()
        conexion.close()
